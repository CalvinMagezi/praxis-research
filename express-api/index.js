...
app.post("/", async (req, res) => {
  ...
  async function rephraseInput(inputString) {
    ...
    // Check if the rephrase is in the cache
    if (rephraseCache.has(inputString)) {
      console.log(`5. Rephrased input found in cache`);
      return rephraseCache.get(inputString);
    }
    ...
    const rephrased = groqResponse.choices[0].message.content;
    rephraseCache.set(inputString, rephrased);
  ...
  for await (const chunk of chatCompletion) {
    if (chunk.choices[0].delta && chunk.choices[0].finish_reason !== "stop") {
      ...
      chatCompletion.done(); // Signal that we're done processing the stream
    }
  }
  ...
  returnFollowUpQuestions
    ? (responseObj.followUpQuestions =
        await generateFollowUpQuestions(responseTotal))
    : null;
  console.log(
